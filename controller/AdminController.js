...
exports.createAdmin = async (req, res) => {
  ...
  if (!REGEX_EMAIL.test(req.body.email)) {
    return res
      .status(400)
      .json({ message: httpRequestMessages.errorRegexEmail });
  }
        ...
        if (error) {
          ...
          if (result[0] != undefined) {
            res
              ...
              .json({
                error: error,
                message: httpRequestMessages.errorEmailConnection,
              });
          } else {
            const { nom, email } = req.body;
            connection.query(
              admin.createAdmin,
              [nom, encryptedPassword, email],
              (error, result) => {
                if (error) {
                  res
                    .status(400)
                  ...
                  if (comparePassword) {
                    ...
                    res
                      .status(401)
                      .json({ error: error, message: httpRequestMessages });
                  }
                }
              }
            );
...
exports.connectAdmin = (req, res) => {
  ...
  if (password.match(password_regex)) {
    ...
    return res
      .status(400)
      .json({ message: httpRequestMessages.errorRegexPassword });
  }
    ...
    async function (error, results, fields) {
      if (error) {
        res
          ...
          .json({
            error: error,
            message: httpRequestMessages.errorChampsEmail,
          });
      } else {
        if (results.length > 0) {
          ...
          if (comparePassword) {
            ...
            res
              ...
              .json({
                error: error,
                message: httpRequestMessages.errorConnectionAdmin,
              });
          }
        } else {
          res
...
exports.getAllAdmin = (req, res) => {
  connection.query(admin.getAllAdmin, (error, result) => {
    if (error) {
      res
        .status(401)
        .json({ error: error, message: httpRequestMessages.errorGetAllAdmin });
    } else {
      res
        .status(200)
        .json({
          result: result,
          message: httpRequestMessages.successGetAllAdmin,
        });
    }
  });
};
...
exports.getAdminById = (req, res) => {
  ...
  connection.query(admin.getAdminById, [id_admin], (error, result) => {
    if (result[0] === undefined) {
      res
        .status(401)
        .json({ error: error, message: httpRequestMessages.errorGetAdminById });
    } else {
      res
        .status(200)
        .json({
          result: result,
          message: httpRequestMessages.successGetAdminById,
        });
    }
  });
};
      ...
      if (error) {
        ...
        if (req.body.password === null || req.body.password === " ") {
          connection.query(
            ...
            [nom, recuperationPassword, email, req.params.id_admin],
            (error, result) => {
              if (error) {
                res
                  .status(400)
                  .json({
                    error: error,
                    message: httpRequestMessages.errorUpdateAdmin,
                  });
              } else {
                res
                  .status(201)
                  .json({
                    result: result,
                    message: httpRequestMessages.successUpdateAdmin,
                  });
              }
            }
          );
          ...
          connection.query(
            admin.updateAdminById,
            [nom, encryptedPassword, recuperationEmail, req.params.id_admin],
            (error, result) => {
              if (error) {
                res
                  .status(400)
          ...
          if (password.match(REGEX_PASSWORD)) {
            connection.query(
              admin.updateAdminById,
              [nom, encryptedPassword, email, req.params.id_admin],
              (error, result) => {
                if (error) {
                  res
                    .status(400)
  ...
  connection.query(admin.deleteAdminById, [id_admin], (error, result) => {
    if (result.affectedRows > 0) {
      res
        ...
        .json({
          result: result,
          message: httpRequestMessages.successDeleteAdmin,
        });
    } else {
      res
        .status(400)
        .json({ error: error, message: httpRequestMessages.errorDeleteAdmin });
    }
  });
};
